...
class InspectionManager(Node):
    ...
    def run_inspection(self, x, y, z,
        ...
        # --- Normalitzacio ---
        self.get_logger().info("5. Normalitzant l'espectre (Reflectancia: GSM / GRF)...")
        ...
        vis_norm = np.divide(vis_gsm, vis_grf, out=np.zeros(SPECTRUM_LENGTH), where=vis_grf != 0)
        nir_norm = np.divide(nir_gsm, nir_grf, out=np.zeros(SPECTRUM_LENGTH), where=nir_grf != 0)
        ...
        # --- Prediccio ML ---
        self.get_logger().info("6. Enviant dades al model de Machine Learning...")
        ...
        if res_ml is None:
            self.get_logger().error("Error: Cap resposta del model de ML (timeout o servei caigut).")
            return

        if res_ml.material.startswith("ERROR"):
            self.get_logger().error(f"Error del model de ML: {res_ml.material}")
            return
        ...
        with open(csv_path, 'a', newline='') as csvfile:
            ...
            if not file_exists:
                ...
                header = [
                    'Daily_Count', 'Measure_Type', 'Date', 'Time',
                ...
                # Calibracions del dia (Daily_Count = 0)
                for calib_data, label in [
                    (res_gds, "GDS"), (res_gdr, "GDR"), (res_grf, "GRF")
                ]:
                    if calib_data is not None and calib_data.vis_ok:
                        row = [0, label, date_str, time_str, x, y, z, "Calibration", ""]
                        row.extend(calib_data.vis_spectrum)
                        row.extend(calib_data.nir_spectrum)
                        writer.writerow(row)
                row_index += 4

            # Mesura GSM normalitzada
            data_row = [
                daily_count, "GSM", date_str, time_str,
            ...
            try:
                ...
                if "hyperspectral_camera" in historial_antic:
                    if isinstance(historial_antic["hyperspectral_camera"], list):
                        dades_historial["hyperspectral_camera"]["measurements"] = \
                            historial_antic["hyperspectral_camera"]
                    elif isinstance(historial_antic["hyperspectral_camera"], dict):
                        dades_historial = historial_antic
            except json.JSONDecodeError:
                self.get_logger().warn(
                    "El fitxer JSON existent estava corrupte. Es crea de nou."
                )
        ...
        self.get_logger().info(f"8. Mesura #{daily_count} guardada correctament!")
        self.get_logger().info(f"   Material   : {res_ml.material}")
        self.get_logger().info(f"   Confianca  : {res_ml.confidence:.3f}")
        self.get_logger().info(f"   CSV        : {csv_path}")
        self.get_logger().info(f"   JSON       : {nom_arxiu_json}")
